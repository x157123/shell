    ...
    async with SEM:
        try:
            ...
            async with asyncssh.connect(
                ...
                await conn.run(f"mkdir -p /home/ubuntu/task/beboundless", check=False)
                ...
                # 4) 执行脚本
                await conn.run(f"chown -R ubuntu:ubuntu /home/ubuntu/task/beboundless", check=False)
                exec_cmd = f"sudo -u ubuntu -i nohup python3 {remote_path!r}"
                if param:
                    exec_cmd += f' --ip "{host}" --param "{param}" > /home/ubuntu/task/beboundless/out.log 2>&1 &'
