...
def main(client, serverId, appId, decryptKey, user, display):
    ...
    # run_command_blocking("sudo chown -R ubuntu:ubuntu /home/ubuntu/.aios")

    logger.info("===== 检测是否已启动 =====")
    status_output = run_command_blocking("/home/ubuntu/.aios/aios-cli status")
    if "Daemon running on" in status_output:
        logger.info("杀掉程序。")
        run_command_blocking("/home/ubuntu/.aios/aios-cli kill")
        time.sleep(10)  # 等待 10 秒
    logger.info("检查结束！")

    # 2. 启动后端服务（后台运行，不阻塞）
    subprocess.Popen("/home/ubuntu/.aios/aios-cli start", shell=True)
    logger.info("后端命令已启动。")
    ...
    run_command_and_print(
        "/home/ubuntu/.aios/aios-cli models add hf:bartowski/Llama-3.2-1B-Instruct-GGUF:"
    ...
    run_command_and_print(
        "/home/ubuntu/.aios/aios-cli infer --model hf:bartowski/Llama-3.2-1B-Instruct-GGUF:"
    ...
    run_command_and_print("/home/ubuntu/.aios/aios-cli hive login", wait_for="Authenticated successfully!")
    ...
    login_output = run_command_blocking("/home/ubuntu/.aios/aios-cli hive whoami")
    public_key = None
    private_key = None
    ...
    run_command_blocking("/home/ubuntu/.aios/aios-cli hive select-tier 5")
    logger.info("select-tier 命令执行完毕。")

    # 8. 执行 hive connect 命令
    logger.info("执行 hive connect 命令...")
    run_command_blocking("/home/ubuntu/.aios/aios-cli hive connect")
    ...
    while True:
        try:
            logger.info("\n===== 积分查询输出 =====")
            login_output = run_command_blocking("/home/ubuntu/.aios/aios-cli hive points")
            points = None
