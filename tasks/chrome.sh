...
#  fi
  ...
  if [ ! -d "$DIR" ]; then
    ...
    wget -q -O /tmp/okx.tar "https://github.com/x157123/ACL4SSR/releases/download/v.1.0.19/okx.tar" || error_exit "钱包下载失败"

    # 解压文件
    log_info "解压文件..."
    tar -xvf /tmp/okx.tar -C "$TARGET_DIR"
...
#  fi
  ...
  if [ ! -d "$DIR" ]; then
    ...
    wget -q -O /tmp/chrome-cloud.tar "https://github.com/x157123/ACL4SSR/releases/download/v.1.0.21/chrome-cloud-v3.tar" || error_exit "钱包下载失败"
