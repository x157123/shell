...
readonly KEPLR_WALLET_URL="https://github.com/x157123/ACL4SSR/releases/download/v1.0.4/keplr.tar"
...
install_wallet() {
  ...
  # 目录路径
  DIR="/home/$USER/extensions/keplr"
  # 文件下载地址
  TARGET_DIR="/home/$USER/extensions/"
  # 判断目录是否存在
  if [ ! -d "$DIR" ]; then
    ...
    if ! curl -sSL "$KEPLR_WALLET_URL" -o /tmp/keplr.tar; then
        log_info "钱包 URL 下载失败，$KEPLR_WALLET_URL"
    fi

    # 解压文件
    log_info "解压文件..."
    tar -xvf /tmp/keplr.tar -C "$TARGET_DIR"
