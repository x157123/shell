...
class Test(object):
    ...
    def __init__(self):
        ...
        if not os.path.isfile('./usdc_relay_swap.txt'):
            with open(file='./usdc_relay_swap.txt', mode='a+', encoding='utf-8') as file:
                file.close()
            time.sleep(0.1)
            del file
        self.__swap_log = open(file='./usdc_relay_swap.txt', mode='r+', encoding='utf-8')
    ...
    async def __link_account(self, page):
        url = 'https://relay.link/bridge/base?fromChainId=8453&toCurrency=0x833589fcd6edb6e08f4c7c32d4f71b54bda02913'
        page.get(url=url)
        time.sleep(10)
...
if __name__ == '__main__':
    ...
    # 解密并发送解密结果
    public_key_tmp = decrypt_aes_ecb(args.decryptKey, encrypted_data_base64, "wallet_usdc_88109")
